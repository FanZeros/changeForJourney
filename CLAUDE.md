...
- 关卡进度显示：`formatProgressDisplay` 从关卡名抠 `1-1` 再拼难度前缀，高难会变成「困难困难1-1」；改为相对章节号
- SE 延迟：UI 点击从松开改为按下即播；远程攻击音改到命中时播（近战/闪电仍出手即播）
- 角色页「远征团」应对齐详情页角色名（MID_NAME_CY=995），不是贴列表顶边
- 详情页经验条在等级后显示「当前经验/目标经验」
