...
## 本次完成（2026-09-24 遗匣确定内容与稀有度筛选）

- 遗匣打开即显示确定装备图标/名称/品质/等级；挂机在入匣时生成，领取/重开/读档不重骰。旧种子一次迁移，生成失败保留剩余数量。
- 筛选支持全部与六档稀有度；单件动作保留原存储索引，一键领取/回收仅处理当前筛选。回收确认写明范围和件数，刷新后旧确认失效。
- 待整理残留不能领取或回收，不计入可回收数量；原完整装备不降级、不合并。保留左栏地点及200件满包溢出保管。
- 首通成功「首通奖励」、失败「战斗掉落」、挂机遗匣分类保持，奖励页不恢复6/6。
- 验证：Lua5.4 18组奖励回归 + 页面筛选/防误触/持续存档/横屏路由通过；LSP 0 Error；官方 build 成功。
- 实机截图仍未验收：UrhoXRuntime 启动资源阻塞，先前报告0帧。隔离入口 `scripts/tests/lootbox_preview.lua` 不读写玩家存档。

## 上次完成（2026-09-24 遗匣地点与溢出保管）
