...
- **不能取消/退出任务**；无论本轮任务成功完成还是遇到阻碍，都必须先汇报结果，再通过 AskUserQuestion 提供明确的下一步选项，不得仅用普通文本中断对话；用户选择后继续处理。不要据此擅自执行未授权操作。
- 分支操作以用户**当前轮次**明确授权为准：本轮从 `workspace925` 创建功能分支，完成后推送功能分支并合并、推送回 `workspace925`；不得推送其他旧分支。访问令牌不得写入仓库或记忆。
...
- 分支禁令以用户当前轮次授权为准；本轮仅推 `workspace925`。
