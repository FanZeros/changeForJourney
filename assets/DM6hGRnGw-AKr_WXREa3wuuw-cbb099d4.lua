...
local function seamBackTarget()
    if ChurchPage.isOpen() then
        return function() ChurchPage.close() end
    elseif BlacksmithPage.isOpen() then
        return function() BlacksmithPage.close() end
    elseif TavernPage.isOpen() then
        return function()
            if TavernPage.isBusy and TavernPage.isBusy() then return end
            TavernPage.close()
        end
    elseif ArenaPage.isOpen() then
        return function() ArenaPage.close() end
    elseif MarketPage.isOpen() then
        return function()
            if MarketPage.isBusy and MarketPage.isBusy() then return end
            MarketPage.close()
        end
    elseif CharacterDetail.isOpen() then
        return function() CharacterDetail.close() end
    end
    return nil
end
