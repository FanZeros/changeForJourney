...
local function drawRosterList(vg)
    ...
    for idx = 1, rosterCount do
        ...
        drawImageCover(vg, cardImg, cx, cy, ROSTER.CARD_W, ROSTER.CARD_H, 1.0)
...
function ChurchPage.handleInput(dx, dy)
    ...
    -- 返回按钮（三栏中缝接管时不响应页内热区）
    if not DrawUtil.useSeamBack() and hitTest(dx, dy, BTN_BACK.CX, BTN_BACK.CY, BTN_BACK.W, BTN_BACK.H) then
        ChurchPage.close()
        return true
    end
...
function ChurchPage.draw(vg)
    ...
    if state.selectedHeroId then
        ...
        drawImageCover(vg, cardImg, cx, cy, CHAR_SLOT.W, CHAR_SLOT.H, 1.0)
        end
    ...
    -- 6. 返回按钮（三栏中缝接管时不画页内那颗）
    if not DrawUtil.useSeamBack() then
    ...
    if state.selectAnim and state.selectedHeroId then
        ...
        nvgSave(vg)
        nvgGlobalAlpha(vg, 1.0)
        drawImageCover(vg, cardImg, curX, curY, ROSTER.CARD_W, ROSTER.CARD_H, 1.0)
        nvgRestore(vg)
    end
