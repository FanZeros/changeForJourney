...
--- 广告观看中：中缝返回键也需拦截，避免半途退出
---@return boolean
function MarketPage.isBusy()
    return state.privAdWatching == true
end
...
function MarketPage.handleInput(dx, dy)
    ...
    -- 返回按钮（三栏中缝接管时不响应页内热区，避免叠两颗）
    if not DrawUtil.useSeamBack() and hitTest(dx, dy, TAB.BACK_CX, TAB.BACK_CY, TAB.BACK_W, TAB.BACK_H) then
