...
--- 招募请求进行中：中缝返回键也需拦截
---@return boolean
function TavernPage.isBusy()
    return pendingGachaPull == true
end
...
function TavernPage.handleInput(dx, dy)
    ...
    -- 返回按钮（三栏中缝接管时不响应页内热区）
    if not DrawUtil.useSeamBack() and hitTest(dx, dy, BTN_BACK_CX, BTN_BACK_CY, BTN_BACK_W, BTN_BACK_H) then
