...
> 更新时间:2026-09-24 | 版本:v2.37-lootbox-fixed-rarity
>
> **当前分支**：`feat/first-clear-reward-cascade`，只推本分支。遗匣为左栏地点，默认显示确定装备，支持稀有度筛选与筛选范围领取/回收。

## 最新交接：确定装备与稀有度筛选

- `LootBoxSystem.addSeed` 保留调用接口，改为入匣时生成完整装备，标记 `source=idle`；溢出 `source=overflow`。领取只移动实例，不随机、不降级。
- `revealLegacy` 供 Schema.onLoad 和 LootBox 门面调用：旧组按数量一次展开；失败保留剩余项，成功项不重骰。9999条旧掉落及10003件混合保管回归通过。
- `LootBoxPage` 顶部“全部+六档品质”，刷新保持筛选、重开默认全部；单件转发 `sourceIndex`。批量回调传 `qualityFilter`（0全部，1..6精确匹配），Boot透传系统。
- 一键回收确认展示范围与件数；待整理项不可操作，也不参与批量回收；刷新使旧确认失效。新增文案进运行时五语，按钮按翻译后文本宽度缩字。
- 当前回归：`lootbox_overflow_test` 18组通过（含真实Boot回调、旧档迁移、重开/读档不重骰、全部品质筛选）；page/horizon输入与持续存档测试通过；LSP 0 Error、官方build成功。
- **实机截图仍阻塞**：运行时engine-startup资源不可用，未将0帧报告认作通过。保留隔离预览入口，待环境恢复后验收。
- POST-1/3：更新本交接、版本行及随行项目偏好；无新增用户画像推断。沿用当前分支提交推送约定，用户当前轮若要求不再提问则不强行弹选项。
