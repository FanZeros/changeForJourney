...
| 版本 | 日期 | 摘要 |
|------|------|------|
| v2.37-lootbox-fixed-rarity | 2026-09-24 | 遗匣默认展示确定装备，挂机入匣即生成、旧档一次迁移；六档稀有度筛选与范围内一键领取/回收，刷新取消旧确认。18组奖励回归及页面/存档/路由、LSP/build通过；实机截图仍受启动资源阻塞。 |
| v2.36-lootbox-location | 2026-09-24 | 遗匣改城镇左栏地点；200件满包奖励完整保管，取消9999种子截断；领取/分解重写及输入防误触；修持续掉落存档。逻辑回归/LSP/build通过，实机截图受运行时启动资源阻塞。 |
