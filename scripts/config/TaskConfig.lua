...
local function nextReward(scale)
    ...
    elseif kind == 3 then
        return { type = "adventure_ticket", amount = 1 + (scale % 3), icon = "image/货币道具/UI_icon_ZMQ_1.png", quality = 5 }
    elseif kind == 4 then
        return { type = "enhance_star", amount = 2 + (scale % 4), icon = "image/货币道具/UI_icon_QH_1.png", quality = 3 }
    elseif kind == 5 then
        return { type = "arcane_dust", amount = scale * 20, icon = "image/货币道具/UI_icon_ASFC.png", quality = 3 }
    elseif kind == 6 then
        return { type = "sweep_ticket", amount = 1 + (scale % 2), icon = "image/货币道具/UI_icon_SDQ.png", quality = 4 }
    elseif kind == 7 then
...
local function addClear(stageId, difficulty, amount)
    ...
    TaskConfig.ACHIEVEMENT[#TaskConfig.ACHIEVEMENT + 1] = {
        ...
        target = 1,
        reward = nextReward(math.floor(amount / 40)),
    }
end
