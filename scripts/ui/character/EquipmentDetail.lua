...
function EquipmentDetail.handleInput(dx, dy)
    ...
    if ExpTable.isBuildingUnlocked("smith", GameState.getLevel())
       and hitTest(dx, dy, btnCX, enhBtnCY, REF_ENH_BTN_W, REF_ENH_BTN_H) then
        ...
        if not EquipmentBag then EquipmentBag = require("ui.character.EquipmentBag") end
        if not BottomNav then BottomNav = require("ui.hud.BottomNav") end
        if not CharacterDetail then CharacterDetail = require("ui.character.CharacterDetail") end
    ...
    if not enhOnly and hitTest(dx, dy, btnCX, btnCY, REF_BTN_W, REF_BTN_H) then
        ...
        local okCP, CP = pcall(require, "ui.character.CharacterPanel")
...
function EquipmentDetail.onActionResult(data)
    ...
    if data.decomposed then
        ...
        if #rewards > 0 then
            require("ui.hud.RewardPopup").show("分解奖励", rewards)
        end
    end
end
