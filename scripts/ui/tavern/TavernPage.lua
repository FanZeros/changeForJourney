...
function TavernPage.init(vg)
    ...
    -- 下半部分
    img.pityBg      = nvgCreateImage(vg, "image/界面底板/酒馆抽卡/UI_JG_SMBJ.png", 0)
...
local function doRecruitDirect(count, forcePayType)
    ...
    RecruitAnim.start(results, function()
        ...
        require("ui.character.HeroScenario").onRecruitResults(results)
    end)
end
...
function TavernPage.onActionResult(data)
    ...
    RecruitAnim.start(data.gachaResults, function()
        ...
        require("ui.character.HeroScenario").onRecruitResults(data.gachaResults)
    end)
end
